...
public class S3Config {
	...
	private String awsAccKey;

	@Value("${jsa.aws.secret_access_key}")
	private String awsSecAccKey;
	
	@Value("${jsa.aws.profile_name}")
	...
	public AmazonS3 s3client(){
		BasicAWSCredentials awsCreds=new BasicAWSCredentials(awsAccKey,awsSecAccKey);
		...
		System.out.println("rigon name is:"+s3Client.getRegionName());
		return s3Client;
	}
	...
	public TransferManager transferManager(){
		...
		try {
			System.out.println("is bucket exists"+tm.getAmazonS3Client().doesBucketExist(bucketName));
	...
	//thread pool for the upload and download
